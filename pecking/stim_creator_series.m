function [Ystim,FS,Bits,Voc_sil, Voc_ID]=stim_creator_series(Stim_list, NStims_tot, Stim_indices, Stim_duration)
if nargin<4
...
while NStims<NStims_tot
    ...
    % to be cut
    if length(Y)>(Fin*Stim_duration/NStims_tot)
        Y = cut_nicely_wav(Y,Fin,Stim_duration/NStims_tot);
    end
    % Retrieve call type
    [~,FileName,~]=fileparts(Stim_name);
    ParseCT1=strfind(FileName,'-');
    ParseCT2=strfind(FileName,'_');
    ParseCT = sort([ParseCT1 ParseCT2]);
    CallType =  FileName(ParseCT(2)+1:ParseCT(2)+2);
    Y = songfilt_call(Y,Fin, 250, 12000, 0, CallType);%bandpass filter from 250Hz to 12000Hz
...
Le = Fin*Stim_duration - VocLe;
RDelay=rand([1,NStims_tot-1]);
RDelay=RDelay./sum(RDelay);
RDelay = [0 RDelay];
Ystim=zeros(Fin*Stim_duration,1);
...
for ss=1:NStims_tot
    ...
    Voc_sil(ss) = floor(RDelay(ss)*Le)/Fin;
end
